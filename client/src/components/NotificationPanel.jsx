...
import { useGetNotificationsQuery, useMarkNotiAsReadMutation } from "../redux/splice/api/userApiSlice";
...
const ICONS = {
  alert: (
    <HiBellAlert className='h-5 w-5 text-gray-400 group-hover:text-indigo-500' />
  ),
  message: (
    <BiSolidMessageRounded className='h-5 w-5 text-gray-400 group-hover:text-indigo-500' />
  ),
};
                ...
                <div className='w-screen max-w-md flex-auto overflow-hidden rounded-3xl border border-white/5 bg-neutral-900 text-sm leading-6 shadow-lg ring-1 ring-gray-900/5'>
                  <div className='p-4'>
                    {data?.slice(0, 5).map((item, index) => (
                      ...
                      >
                        <div className='mt-1 h-8 w-8 flex items-center justify-center rounded-lg bg-black group-hover:bg-gray-400'>
                          {ICONS[item.notiType]}
                        </div>
                  ...
                  <div className='grid grid-cols-2 divide-x bg-gray-900'>
                    {callsToAction.map((item) => (
                      <Link
                        ...
                        className='flex items-center justify-center gap-x-2.5 p-3 font-semibold text-blue-600 hover:bg-gray-400'
                      >
                        {item.name}
                      </Link>
                    ))}
